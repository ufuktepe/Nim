...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        `state`, return 0.
        """
        # Choose the best available action and return its Q-value
    ...
    def choose_action(self, state, epsilon=True):
        ...
        """
        # If epsilon is true pick a random action with probability self.epsilon
        if epsilon and self.epsilon > random.random():
            return random.choice(list(Nim.available_actions(state)))

        # Otherwise, pick the best available action
